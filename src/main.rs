    ...
    fn execute(self) -> Result<(), Error> {
        match self {
            Command::Embed {
                ...
                let sekreto = Sekreto::try_from(dosierenhavo.as_path())?;
                ...
                for (pikselo, b) in bildo.pixels_mut().zip(ensxipigxis.iter()) {
                    ...
                    *pikselo = image::Rgba(cxifri_bajton(*rgba, *b));
                }
            ...
            Command::Extract {
                ...
                let ensxipigxis_lon = usize::from_le_bytes(
                    <Vec<u8> as TryInto<[u8; USIZE_LEN]>>::try_into(
                        (0..USIZE_LEN)
                            ...
                            .map(|image::Rgba(rgba)| decxifri_bajton(*rgba))
                            .collect::<Vec<u8>>(),
                    )
                    .unwrap(),
                );
                ...
                let Sekreto { dosiero, enhavo } = Sekreto::try_from(
                    pikseloj
                        ...
                        .map(|(image::Rgba(rgba), _)| decxifri_bajton(*rgba))
                        .collect::<Vec<_>>(),
                )?;
...
fn cxifri_bajton(rgba: [u8; 4], bajto: u8) -> [u8; 4] {
    let [r, g, b, a] = rgba;
    [
        (r & 0xFC) + ((bajto & 0xC0) >> 6),
        (g & 0xFC) + ((bajto & 0x30) >> 4),
...
fn decxifri_bajton(rgba: [u8; 4]) -> u8 {
    let [r, g, b, a] = rgba;
